...
/**
 * Defines the policy based on which [[com.ctrip.atom.util.logging.RollingFileAppender]] will
 * generate rolling files.
 */
private[atom] trait RollingPolicy {
...
/**
 * Defines a [[com.ctrip.atom.util.logging.RollingPolicy]] by which files will be rolled
 * over at a fixed interval.
 */
private[atom] class TimeBasedRollingPolicy(
    ...
    if (checkIntervalConstraint && rolloverIntervalMillis < MINIMUM_INTERVAL_SECONDS * 1000L) {
        logWaring(s"Rolling interval [${rolloverIntervalMillis / 1000L} seconds] is too small. " +
...
private[atom] object TimeBasedRollingPolicy {
 ...
 */
private[atom] class SizeBasedRollingPolicy(
    ...
    if (checkSizeConstraint && rolloverSizeBytes < MINIMUM_SIZE_BYTES) {
        logWaring(s"Rolling size [$rolloverSizeBytes bytes] is too small. " +
...
private[atom] object SizeBasedRollingPolicy {
