...
import com.ctrip.atom.util.logging.RollingFileAppender._
 ...
 */
private[atom] class RollingFileAppender(
        ...
        try {
            ...
            if (activeFile.exists) {
                if (!rolloverFile.exists) {
                    ...
                    logWaring(s"Rollover file $rolloverFile already exists, " +
                      s"rolled over $activeFile to file $altRolloverFile")
                    Files.move(activeFile, altRolloverFile)
                }
            } else {
                logWaring(s"File $activeFile does not exist")
            }
        }
    }
...
/**
 * Companion object to [[com.ctrip.atom.util.logging.RollingFileAppender]]. Defines
 * names of configurations that configure rolling file appenders.
 */
private[atom] object RollingFileAppender {
